...
class TestAuthService:
    @pytest.mark.parametrize(
        ...
        [
            (
                {"email": "a@a.com", "password": "123", "re_password": "123"},
                status.HTTP_201_CREATED,
            ),
            (
                {"email": "a@.a", "password": "123", "re_password": "123"},
                status.HTTP_422_UNPROCESSABLE_ENTITY,
            ),
            (
                {"email": "a@.a", "password": "123", "re_password": "1"},
                status.HTTP_422_UNPROCESSABLE_ENTITY,
            ),
        ],
    )
    async def test_register_user_async(
        self, ac: AsyncClient, request_data, status_code
    ):
    ...
    async def test_refresh_tokens(
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            assert data.get("refresh_token") is not None
            response: Response = await ac.post(
                "/auth/refresh/", json=f"{data.get("refresh_token")}"
            )
    ...
    async def test_logout_device(
        ...
        # Get devices
        response: Response = await ac.get(
            "/auth/devices/", headers={"Authorization": f"Bearer {tokens.access_token}"}
        )
    ...
    async def test_logout_all_devices(
        ...
        """Logout from all devices scenarios."""
        user: SUserCreate = random_user[0]
        access_token = random_user[1].access_token

        # Invalid token scenarios
        response: Response = await ac.post(
            "/auth/devices/logout-all/",
            headers={"Authorization": f"Bearer invalid_token"},
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

        # Create second device
        response_data = {"email": user.email, "password": user.password}
        await ac.post("/auth/login/", json=response_data)
